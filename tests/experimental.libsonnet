...

{
  ...
  BaseTpuVmMixin:: self.BaseTpuVmTest {
    local config = self, # refer to BaseTpuVmMixin
  ...
  BaseCudaVmTest:: {
    ...
    cudaSettings+: {
      local cudaSettings = self,

      softwareVersion: 'v2-nightly',

      // Startup script in TPU VM metadata.
      cudaVmStartupScript: 'echo Running startup script',

      // Amount of time to sleep after TPU is READY.
      cudaVmCreateSleepSeconds:
        if config.accelerator.version <= 3 then
          60
        else
          90,
    ...
    podTemplate+:: {
      spec+: {
        ...
        initContainerMap+:: {
          'create-cuda': {
            image: 'google/cloud-sdk',
            local cudaCreateSettings = {
              acceleratorName: std.escapeStringBash(config.accelerator.name),
              softwareVersion: std.escapeStringBash(config.cudaSettings.softwareVersion),
              startupScript: std.escapeStringBash(config.cudaSettings.cudaVmStartupScript),
              sleepTime: config.cudaSettings.cudaVmCreateSleepSeconds,
              testName: std.strReplace(config.testName, '.', '-'),
            },
            command: utils.scriptCommand(|||
              project=$(curl -sS "http://metadata.google.internal/computeMetadata/v1/project/project-id" -H "Metadata-Flavor: Google")
  ...
  BaseCudaVmMixin:: self.BaseCudaVmTest {
    local config = self,
    ...
    podTemplate+:: {
      spec+: {
        # xw32: where is config.timeout defined?
        # defined in https://github.com/GoogleCloudPlatform/ml-testing-accelerators/blob/master/templates/mixins.libsonnet
        activeDeadlineSeconds: config.timeout,
        containerMap+:: {
          train+: {
            image: 'google/cloud-sdk',
